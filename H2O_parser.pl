...
while(my $line = <SITE2>){
	...
	for(my $i=0;$i<scalar(@site1);$i++){
		...
		if($C1[$i] == $fields[2] && $P1[$i] == $fields[7] && $S1[$i] == $fields[6] && $N1[$i] == $fields[5] && ($O1[$i]-$fields[4])/($H1[$i]-$fields[3]) == 0.5){
			my $o_diff = $O1[$i]-$fields[4];
			print OUT "$site1[$i]	$fields[0]	$mass1[$i]	$fields[1]	$El_comp_1[$i]	$fields[8]	$Class_1[$i]	$fields[9]	$MolForm_1[$i]	$fields[10]	$o_diff\n";
		}
	}
}
close SITE2;
